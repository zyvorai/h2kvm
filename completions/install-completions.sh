...
# Shell completion installation script for h2kvm
...
print_header() {
    echo -e "${BLUE}============================================${NC}"
    echo -e "${BLUE}h2kvm Shell Completion Installer${NC}"
    echo -e "${BLUE}============================================${NC}"
    echo
}
...
install_bash() {
    ...
    if [ -w /etc/bash_completion.d/ ]; then
        sudo cp "${SCRIPT_DIR}/h2kvm.bash" /etc/bash_completion.d/h2kvm
    ...
    # Fallback to user-local installation
        ...
        cp "${SCRIPT_DIR}/h2kvm.bash" "${HOME}/.bash_completion.d/h2kvm"
        print_success "Installed user-local bash completion to ~/.bash_completion.d/"

        # Add sourcing to .bashrc if not already present
        if ! grep -q ".bash_completion.d/h2kvm" "${HOME}/.bashrc" 2>/dev/null; then
            echo "" >> "${HOME}/.bashrc"
            echo "# h2kvm completion" >> "${HOME}/.bashrc"
            echo "[ -f ~/.bash_completion.d/h2kvm ] && source ~/.bash_completion.d/h2kvm" >> "${HOME}/.bashrc"
            print_info "Added source line to ~/.bashrc"
        fi
        echo "  Run: source ~/.bashrc"
    else
        print_error "Could not find or create bash completion directory"
        print_info "Manual installation: source ${SCRIPT_DIR}/h2kvm.bash"
        return 1
    fi
}
...
install_zsh() {
    ...
    if [ -w /usr/share/zsh/site-functions/ ]; then
        sudo cp "${SCRIPT_DIR}/h2kvm.zsh" /usr/share/zsh/site-functions/_h2kvm
        print_success "Installed system-wide zsh completion to /usr/share/zsh/site-functions/"
        echo "  Run: exec zsh"
    # Fallback to user-local installation
    else
        mkdir -p "${HOME}/.zsh/completion"
        cp "${SCRIPT_DIR}/h2kvm.zsh" "${HOME}/.zsh/completion/_h2kvm"
        ...
        if ! grep -q "fpath.*zsh/completion" "${HOME}/.zshrc" 2>/dev/null; then
            echo "" >> "${HOME}/.zshrc"
            echo "# h2kvm completion" >> "${HOME}/.zshrc"
...
install_fish() {
    ...
    if [ -w /usr/share/fish/vendor_completions.d/ ]; then
        sudo cp "${SCRIPT_DIR}/h2kvm.fish" /usr/share/fish/vendor_completions.d/
    ...
    # Fallback to user-local installation
        ...
        cp "${SCRIPT_DIR}/h2kvm.fish" "${HOME}/.config/fish/completions/"
