...
#
# H2KVM LUKS Auto-Unlock - Initramfs Script
#
...
mapper_name() {
    ...
    echo "h2kvm-$(echo "$device" | sha256sum | cut -c1-12)"
}
...
try_tpm_unlock() {
    device="$1"
    mapper="$2"
    handle="${H2KVM_TPM_HANDLE:-0x81000010}"
...
try_keyfile_unlock() {
    device="$1"
    mapper="$2"
    keyfile="${H2KVM_LUKS_KEYFILE:-/etc/h2kvm/luks.key}"
...
if [ "${0##*/}" = "h2kvm-luks-unlock.sh" ]; then
    main
fi
