...
```yaml
# /etc/h2kvm/profiles/my_org.yaml
...
```yaml
# config.yaml
profiles_directory: "/etc/h2kvm/profiles"
```
...
# Load custom profile
profile = loader.load_profile("my_org", custom_profile_path="/etc/h2kvm/profiles")
...
h2kvm --config manifest-testing.json

# Then production migration
h2kvm --config manifest-production.json
```
