...
from h2kvm.cli.system_config import load_system_config, merge_with_system_config
...
def build_parser() -> argparse.ArgumentParser:
    """Build the top-level h2kvm argument parser with all option groups registered."""
    epilog = _build_epilog()

    p = argparse.ArgumentParser(
        description=c("h2kvm: Ultimate VMware → KVM/QEMU Converter + Fixer", "green", ["bold"]),
...
def parse_args_with_config(
    ...
    try:
        ...
        import argcomplete  # optional dependency, kept lazy so h2kvm imports fine without it installed
