...
def resolve_guest_profile(args: Any, *, vm_name: str = "") -> GuestProfile:
    ...
    if distro:
        labels["h2kvm.io/os-distro"] = distro
    if version:
        labels["h2kvm.io/os-version"] = version
    if pretty:
        annotations["h2kvm.io/os-pretty-name"] = str(pretty)[:256]
...
def apply_guest_metadata(
    ...
    """Merge standard h2kvm / v9s guest-os labels."""
    labels["h2kvm.io/guest-os"] = profile.family
    labels["v9s.io/guest-os"] = profile.family
    annotations["h2kvm.io/guest-os"] = profile.family
    if profile.os_variant:
        annotations["h2kvm.io/os-variant"] = profile.os_variant
