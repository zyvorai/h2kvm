...
# h2kvm/deployers/kubernetes.py
"""
Kubernetes/k3s deployment module for h2kvm.
...
from h2kvm.converters.qemu.converter import Convert, run_qemu_img_convert
from h2kvm.core.exceptions import DiskConversionError, InfrastructureError
from h2kvm.core.firmware_resolver import serial_indicates_boot_failure
...
CONTAINER_DISK_QCOW2_MAX_BYTES = int(
    os.environ.get("H2KVM_CONTAINER_DISK_MAX_BYTES", str(4 * 1024 * 1024 * 1024))
)
...
class KubernetesDeployer:
    ...
    def _build_container_disk(self, qcow2_path: str) -> str | None:
        ...
        image_tag = f"localhost/h2kvm/{self.vm_name}:latest"
    ...
    def _upload_image(self, qcow2_path: str):
        ...
        cp_timeout = max(14400, int(os.environ.get("H2KVM_KUBECTL_CP_TIMEOUT", "14400")))
        self.logger.info(
            "kubectl cp timeout: %ds (set H2KVM_KUBECTL_CP_TIMEOUT to override)",
            cp_timeout,
        )
    ...
    def _log_windows_rdp_access_hints(self, *, guest_ip: str | None = None) -> None:
        ...
        self.logger.info("─" * 60)
        self.logger.info("Windows RDP access (after first guest boot + h2kvm-firstboot)")
        self.logger.info(
            "  Firstboot log on guest: C:\\Windows\\Temp\\h2kvm-firstboot.log "
            "(marker: C:\\h2kvm\\firstboot.done)"
        )
        self.logger.info(
    ...
    def _deploy_novnc(self) -> str | None:
        ...
        novnc_pod = {
            ...
            "metadata": {
                ...
                "labels": {
                    "app": novnc_name,
                    "h2kvm.io/component": "novnc",
                    "h2kvm.io/vm": self.vm_name,
                },
            },
            "spec": {
