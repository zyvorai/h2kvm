...
from h2kvm.fixers.windows.registry.encoding import (
    _close_best_effort,
    _commit_best_effort,
    _ensure_child,
    _set_dword,
...
def enable_msi_interrupts(
    ...
    # in h2kvm/fixers/windows/performance/trim.py's enable_trim_discard()
    ...
    try:
        ...
        script_path = f"{root}/h2kvm/performance/msi-verify.ps1"
        try:
...
def _generate_msi_verification_script(devices: list[str]) -> str:
    ...
    # h2kvm/fixers/windows/activedirectory/cleanup.py's
...
$LogFile = "C:\\h2kvm\\performance\\msi-verify.log"
