...
Usage:
    from h2kvm.fixers.windows.virtio_injector import VirtIOOfflineInjector
...
# Shared with h2kvm/fixers/windows/virtio_stage.py (imports this constant directly
...
# verbatim by h2kvm/fixers/windows/virtio_stage.py, which already imports from
...
class VirtIOOfflineInjector:
    ...
    def _mount_image(self) -> None:
        ...
        self._win_mount = Path(tempfile.mkdtemp(prefix="h2kvm-inject-"))
        subprocess.run(
    ...
    def _mount_iso(self) -> None:
        ...
        self._iso_mount = Path(tempfile.mkdtemp(prefix="h2kvm-iso-"))
        subprocess.run(
    ...
    def promote_viostor_boot_start(qcow2_path: str | Path) -> None:
        ...
        try:
            ...
            mount_point = Path(tempfile.mkdtemp(prefix="h2kvm-promote-"))
            subprocess.run(
