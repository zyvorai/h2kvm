...
from h2kvm.fixers.windows.registry.mount import _ensure_windows_root
from h2kvm.fixers.windows.registry.software import append_devicepath_software_hive
from h2kvm.fixers.windows.registry.system import edit_system_hive
...
def _virtio_preflight(self, g: guestfs.GuestFS) -> tuple[Path | None, dict[str, Any] | None]:
    ...
    if not virtio_dir:
        from h2kvm.fixers.windows.virtio_warning import VIRTIO_WIN_RPM_ISO
    ...
    if not virtio_dir:
        ...
        try:
            from h2kvm.fixers.windows.bitlocker import check_bitlocker_before_migration
        ...
        # RDP Verification
        try:
            from h2kvm.fixers.windows.rdp import log_rdp_precheck_summary, verify_rdp_enabled
        ...
        try:
            from h2kvm.fixers.windows.firewall import stage_firewall_export_script
        ...
        try:
            from h2kvm.fixers.windows.virtio_warning import warn_no_virtio_drivers
...
def _virtio_stage_packages(
    ...
    staging_root = "/h2kvm/drivers/virtio"
    devicepath_append = r"%SystemDrive%\h2kvm\drivers\virtio"
...
def _virtio_stage_manual_setup_cmd(self, g: guestfs.GuestFS, result: dict[str, Any]) -> None:
    ...
    for staged in result["packages_staged"]:
        ...
        if inf:
            ...
            script_content += (
                f'pnputil /add-driver "C:\\h2kvm\\drivers\\virtio\\{service_name}\\{inf_name}" '
                "/install\r\n"
            )
...
def _virtio_stage_guest_tools_installers(
    ...
    firstboot:
    C:\\h2kvm\\drivers\\virtio\\guest-agent\\virtio-win-guest-tools.exe.
    C:\\h2kvm\\drivers\\virtio\\guest-agent\\virtio-win-gt-x64.msi.
    C:\\h2kvm\\drivers\\virtio\\guest-agent\\virtio-win-gt-x86.msi.
    C:\\h2kvm\\drivers\\virtio\\guest-agent\\qemu-ga-x86_64.msi.
    """
    ...
    with _step(logger, "🧩 Update SOFTWARE DevicePath (PnP discovery)"):
        try:
            ...
            msg = (
                ...
                f"After boot, manually run: pnputil /add-driver C:\\h2kvm\\drivers\\virtio\\*\\*.inf /install"
            )
            result["warnings"].append(msg)
            _log(logger, logging.WARNING, "%s", msg)
...
def _virtio_provision_firstboot(
    ...
    if not has_virtio_packages:
        ...
        staging_root = staging_root or "/h2kvm/drivers/virtio"

    log_path_guestfs = f"{paths.temp_dir}/h2kvm-firstboot.log"
    logger.debug(
        "Provisioning firstboot: packages_staged=%d, service=h2kvm-firstboot, "
    ...
    with _step(logger, "🛠️ Provision firstboot service (pnputil /install + logging)"):
        try:
            ...
            msg = (
                ...
                f"After boot, manually run: C:\\h2kvm\\setup.cmd"
            )
            result["warnings"].append(msg)
            _log(logger, logging.WARNING, "%s", msg)
