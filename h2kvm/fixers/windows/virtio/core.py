...
_VIRTIO_CACHE_DIR = Path("/var/lib/h2kvm/virtio-win-extracted")
...
def _materialize_virtio_source(self, virtio_path: Path):
    ...
    The cache is stored at /var/lib/h2kvm/virtio-win-extracted/ and is
    ...
    for store_type, store_path in bcd_stores.items():
        try:
            if g.is_file(store_path):
                ...
                if not dry_run:
                    ts = U.now_ts()
                    backup_path = f"{store_path}.backup.h2kvm.{ts}"
                    try:
...
def _virtio_finalize(
    ...
    result["notes"] += [
        ...
        r"Staging: payload staged under C:\h2kvm\drivers\virtio and installed via firstboot service (pnputil).",
