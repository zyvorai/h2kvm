...
- Writes detailed logs to Windows\\Temp\\h2kvm-firstboot.log
...
_DEFAULT_GUEST_DIR = "/h2kvm"
_DEFAULT_DRIVER_STAGE_DIR = "/h2kvm/drivers/virtio"
_DEFAULT_LOG_PATH = "/Windows/Temp/h2kvm-firstboot.log"
_DEFAULT_MARKER_PATH = "/h2kvm/firstboot.done"
    ...
    *,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            if not dry_run:
                ts = U.now_ts()
                backup_path = f"{system_hive_path}.h2kvm.backup.{ts}"
...
if %ERRORLEVEL%==0 (
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$LOG=$env:LOG;" ^
    "try {" ^
    " 'Registering h2kvm event source...' | Out-File -Append -Encoding ascii $LOG;" ^
    " if(-not [System.Diagnostics.EventLog]::SourceExists('h2kvm')) {" ^
    "  [System.Diagnostics.EventLog]::CreateEventSource('h2kvm','Application');" ^
    "  'Event source registered: h2kvm' | Out-File -Append -Encoding ascii $LOG;" ^
    " } else {" ^
    "  'Event source already exists: h2kvm' | Out-File -Append -Encoding ascii $LOG;" ^
    " }" ^
    " 'Writing conversion completion event...' | Out-File -Append -Encoding ascii $LOG;" ^
    " $evt=New-Object System.Diagnostics.EventLog('Application');" ^
    " $evt.Source='h2kvm';" ^
    " $message='H2KVM first boot initialization completed successfully. VMware to KVM conversion finalized.';" ^
...
if %ERRORLEVEL%==0 (
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "$LOG=$env:LOG;" ^
    "try {" ^
    " $metadataDir=\"$env:SystemDrive\h2kvm\";" ^
...
def _firstboot_windows_paths(service_name: str) -> _FirstbootWinPaths:
    ...
    #   guestfs /h2kvm               -> Windows C:\h2kvm
    #   guestfs /h2kvm/drivers/virtio-> Windows C:\h2kvm\drivers\virtio
    #   guestfs /Windows/Temp/...        -> Windows C:\Windows\Temp\...
    win_guest_dir = r"%SystemDrive%\h2kvm"
...
if exist "%MARKER%" (
  echo h2kvm firstboot marker exists: %MARKER%>> "%LOG%"
...
echo ==================================================>> "%LOG%"
echo ==================================================>> "%LOG%"
echo    H2KVM Enterprise First Boot Initialization
echo ==================================================>> "%LOG%"
echo ==================================================>> "%LOG%"
...
if exist "%ProgramData%\Microsoft\Windows\Start Menu\Programs\Startup\h2kvm-firstboot.bat" (
  del /f /q "%ProgramData%\Microsoft\Windows\Start Menu\Programs\Startup\h2kvm-firstboot.bat" >> "%LOG%" 2>&1
...
echo for h2kvm events (Event ID 1000)>> "%LOG%"
echo.>> "%LOG%"
...
def _firstboot_service_cmdline(service_name: str = "h2kvm-firstboot") -> str:
    """Return the literal Windows path to the firstboot script.

    Uses C:\\h2kvm (literal, no env vars) because rhsrvany reads
    Parameters\\CommandLine as REG_SZ — environment variables are NOT expanded.
    """
    return rf'"C:\h2kvm\{service_name}.cmd"'
...
def provision_firstboot_payload_and_service(
    ...
    *,
    ...
    service_name: str = "h2kvm-firstboot",
    guest_dir: str = _DEFAULT_GUEST_DIR,
    log_path: str = _DEFAULT_LOG_PATH,
    driver_stage_dir: str = _DEFAULT_DRIVER_STAGE_DIR,
    ...
    End-to-end enterprise firstboot provisioning (policy-driven paths for stability):
      ...
      2) Upload firstboot.cmd to /h2kvm/firstboot.cmd with enterprise features
    ...
    svc_res = _add_firstboot_service_system_hive(
        ...
        system_hive_path,
        service_name=service_name,
        display_name="h2kvm First Boot Driver Installer",
        cmdline=cmdline,
        start=2,
        use_rhsrvany=rhsrvany_uploaded,
        description="One-shot first boot installer for h2kvm staged drivers; writes log to Windows\\Temp.",
    )
    ...
    try:
        ...
        run_cmd = f'cmd.exe /c "C:\\h2kvm\\{service_name}.cmd"'
    ...
    if not dry_run:
        try:
            startup_bat = (
                "@echo off\r\n"
                f'if exist "C:\\h2kvm\\{service_name}.done" goto :eof\r\n'
                ...
                f'        echo done > "C:\\h2kvm\\{service_name}.done"\r\n'
                '        del "%~f0"\r\n'
                "        exit /b 0\r\n"
                "    )\r\n"
                ...
                'msg * /time:30 "h2kvm: VirtIO drivers need to be installed. Please open the '
                'CD-ROM drive (D:) and run virtio-win-guest-tools.exe" 2>nul\r\n'
                "if exist D:\\ explorer.exe D:\\\r\n"
            )
            startup_guest_path = (
                "/ProgramData/Microsoft/Windows/Start Menu/Programs/Startup/h2kvm-firstboot.bat"
            )
            _mkdir_p_guest(
                logger,
                g,
    ...
    results["notes"] += [
        ...
        "Log file: C:\\Windows\\Temp\\h2kvm-firstboot.log (guestfs: /Windows/Temp/h2kvm-firstboot.log).",
        "Drivers staged under: C:\\h2kvm\\drivers\\virtio (guestfs: /h2kvm/drivers/virtio).",
        "Completion marker: C:\\h2kvm\\firstboot.done (prevents re-runs if service delete fails).",
        "Conversion metadata: C:\\h2kvm\\metadata.json (tracks applied features and timestamps).",
        "Windows Event Log: Application log with source 'h2kvm' (Event ID 1000 on completion).",
    ...
    if install_qemu_guest_agent:
        results["notes"].append(
            "✅ QEMU Guest Agent: MSI installation from staged drivers (C:\\h2kvm\\drivers\\virtio\\guest-agent\\)"
        )
    ...
    if event_log_integration:
        results["notes"].append(
            "✅ Event Log integration: h2kvm event source registered + completion event (ID 1000)"
        )
    ...
    results["notes"].append(
        ...
        "  1. Check log: type C:\\Windows\\Temp\\h2kvm-firstboot.log\n"
        "  2. Check Event Viewer: Application log → Source: h2kvm → Event ID 1000\n"
        "  3. Check metadata: type C:\\h2kvm\\metadata.json\n"
        "  4. Check QEMU-GA: sc query QEMU-GA\n"
        "  5. Check marker: dir C:\\h2kvm\\firstboot.done"
    )
...
def log_firstboot_provision_summary(
    ...
    fb: dict[str, Any],
    *,
    guest_log_windows_path: str = r"C:\Windows\Temp\h2kvm-firstboot.log",
    ...
    logger.info(
        "Firstboot: installed h2kvm-firstboot (virtio_packages=%d, rdp=%s, qemu_ga=%s, "
        "enhanced_virtio=%s)",
        virtio_packages,
    ...
    logger.info(
        "  Runs on first guest boot — log: %s | marker: C:\\h2kvm\\firstboot.done",
