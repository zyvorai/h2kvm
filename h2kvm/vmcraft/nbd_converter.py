...
class DiskConverter:
    ...
    def _check_disk_space(self, virtual_size: int, actual_size: int, create_type: str) -> None:
        ...
        if avail_bytes < needed:
            allow = os.getenv("H2KVM_ALLOW_LOW_DISK_SPACE", "").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            if not allow:
                raise RuntimeError(
                    ...
                    f"  export H2KVM_ALLOW_LOW_DISK_SPACE=1"
                )
            ...
            self.logger.warning("⚠️  H2KVM_ALLOW_LOW_DISK_SPACE=1 detected - proceeding anyway")
