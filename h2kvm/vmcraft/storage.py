...
from h2kvm.core.constants import DELAY_DEVICE_SETTLE
...
try:
    from .namespace_lvm import H2KVM, H2KVMError
...
class LVMActivator:
    ...
    def _create_isolated_lvm_env(logger: logging.Logger) -> tuple[dict[str, str], Path]:
        ...
        lvm_system_dir = Path(tempfile.gettempdir()) / f"h2kvm-lvm-{os.getpid()}"
    ...
    def _get_lvm_device_filter(nbd_device: str) -> str:
        ...
        # h2kvm/vmcraft/lvm_executor.py -- structurally similar by
        ...
        try:
            ...
            if vgs_to_activate:
                ...
                allow_host_activation = os.getenv(
                    "H2KVM_ALLOW_HOST_VG_ACTIVATION", ""
                ).strip().lower() in {
                    "1",
                    "true",
                    "yes",
                ...
                logger.warning("⚠️  H2KVM_ALLOW_HOST_VG_ACTIVATION=1 detected - safety override enabled")
    ...
    def activate_namespace(
        ...
        try:
            ...
            # H2KVM manages its own module-level logger; it has no logger constructor param.
            engine = H2KVM(
                image=image_path,
                nbd=nbd_device,
                mount_options=["ro"],
...
class LUKSUnlocker:
    ...
    def unlock(self, nbd_device: str | None = None) -> dict[str, Any]:
        ...
        # h2kvm/fixers/offline_fixer.py (_unlock_luks_devices) --
...
class StorageStackActivator:
    ...
    def _settle_udev(self) -> None:
        ...
        # h2kvm/vmcraft/nbd.py's VG-activation step -- structurally
    ...
    def _deactivate_zfs(self) -> None:
        """Export only ZFS pools that were imported by h2kvm."""
        if not _has_command("zpool"):
            return
    ...
    def _cleanup_temp_mounts(self) -> None:
        ...
        try:
            run_sudo(
                self.logger,
                ["sh", "-c", "umount -R /tmp/h2kvm-guestfs-* 2>/dev/null || true"],
                check=False,
                capture=True,
                failure_log_level=logging.DEBUG,
            )
