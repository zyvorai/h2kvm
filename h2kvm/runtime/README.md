...
h2kvm daemon --manifest /path/to/manifest.yaml

# Or NBD preparation daemon
h2kvm daemon --nbd-prep --device /dev/nbd0
```
...
h2kvm worker start --queue redis://localhost:6379

# Submit jobs via API/CLI
h2kvm submit-job --disk /path/to/disk.qcow2
```
...
```python
from h2kvm.runtime.daemon import ManifestWorkflowDaemon
...
```python
from h2kvm.runtime.worker.engine import WorkerEngine
from h2kvm.runtime.worker.schemas import JobSpec, OperationType
...
@kopf.on.create('h2kvm.io', 'v1alpha1', 'migrationjobs')
...
h2kvm_migration_jobs_total
h2kvm_migration_jobs_running
h2kvm_migration_jobs_succeeded
h2kvm_migration_jobs_failed
...
# Worker metrics
h2kvm_worker_nodes_total
h2kvm_worker_jobs_assigned
```
...
daemon:
  manifest_dir: /var/lib/h2kvm/manifests
...
systemctl status h2kvm-daemon

# Check logs
journalctl -u h2kvm-daemon -f

# Validate manifest
h2kvm validate-manifest /path/to/manifest.yaml
```
...
# Verify job schema
h2kvm validate-job job.json
```

### Operator CRD Issues
```bash
# Check operator logs
kubectl logs -n h2kvm-system deployment/operator
