...
class WorkerMetrics:
    ...
    - h2kvm_migration_failures_total: Total migration failures
    - h2kvm_worker_info: Worker information (static)
    - h2kvm_worker_jobs_active: Current active jobs
    ...
    def __init__(self, worker_id: str, enabled: bool = True):
        ...
        self.migration_total = Counter(
            "h2kvm_migration_total", "Total number of migrations", ["worker_id", "operation", "status"]
        )
        ...
        self.worker_info = Info("h2kvm_worker", "Worker information")

        # Active jobs
        self.jobs_active = Gauge(
            "h2kvm_worker_jobs_active", "Number of currently active jobs", ["worker_id"]
        )

        # Disk image size distribution
        self.vmdk_size = Histogram(
            "h2kvm_disk_image_size_bytes",
            "Disk image size distribution",
            ["worker_id"],
            buckets=[
                1e9,  # 1 GB
        ...
        self.temp_usage = Gauge(
            "h2kvm_conversion_temp_usage_bytes", "Temporary storage usage in bytes", ["worker_id"]
        )

        self.temp_capacity = Gauge(
            "h2kvm_conversion_temp_capacity_bytes", "Temporary storage capacity in bytes", ["worker_id"]
        )
