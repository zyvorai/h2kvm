...
DOCS_BASE = "https://github.com/ssahani/h2kvm/blob/main/docs"
...
def missing_dependency_error(package: str, install_cmd: str | None = None) -> str:
    ...
    if package == "pyvmomi":
        helper.add_solution("Or install with vSphere support: pip install h2kvm[vsphere]")
        ...
        helper.add_solution("Or install with Azure support: pip install h2kvm[azure]")
...
def vm_not_found_error(vm_name: str, vcenter: str | None = None) -> str:
    ...
    helper.add_solution("List all accessible VMs: h2kvm vsphere --vs-action list-vms")
...
def boot_failure_error(reason: str | None = None) -> str:
    ...
    helper.add_example(
        "h2kvm --config your-config.yaml --regen-initramfs --fstab-mode stabilize-all --fix-grub"
    )
...
def permission_error(path: str, operation: str = "access") -> str:
    ...
    helper.add_solution("Run with sudo: sudo python -m h2kvm ...")
