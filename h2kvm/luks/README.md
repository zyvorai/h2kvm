...
Boot Flow:
  ...
  Kernel + Initramfs
    ↓
  dracut: h2kvm-luks-unlock
    ↓
  Try unlock sources (TPM → Vault → Keyfile)
    ↓
  cryptsetup open /dev/sda1 h2kvm-xxx
    ↓
  LVM activation
    ↓
...
```bash
# Copy dracut module
cp -r contrib/dracut/90h2kvm-luks /usr/lib/dracut/modules.d/
...
```bash
export H2KVM_TPM_HANDLE=0x81000010
export H2KVM_VAULT_ADDR=https://vault:8200
export H2KVM_VAULT_TOKEN=s.xxxxxx
export H2KVM_VAULT_PATH=secret/h2kvm/luks
export H2KVM_LUKS_KEYFILE=/etc/h2kvm/luks.key
```
...
LUKS Device Status:
  /dev/sda1: ✓ UNLOCKED
    Mapper: /dev/mapper/h2kvm-abc123def456
  /dev/sdb1: ✗ LOCKED
```
...
```bash
# Boot logs
journalctl -b | grep h2kvm-luks

# Initramfs logs
journalctl -b 0 -u h2kvm-luks-unlock
```
...
h2kvm-luks seal /root/luks.key --pcr 0

# Seal to firmware + bootloader (PCRs 0,1,2,3)
h2kvm-luks seal /root/luks.key --pcr 0 1 2 3

# Seal to firmware + Secure Boot (PCRs 0,7)
h2kvm-luks seal /root/luks.key --pcr 0 7
```

### Vault with Kubernetes Auth

```python
from h2kvm.luks.unlocker import VaultKeySource
...
```python
from h2kvm.luks import LUKSUnlocker
...
| Feature | H2KVM | Clevis | systemd-cryptenroll |
|---------|-----------|---------|---------------------|
| TPM2 Support | ✅ | ✅ | ✅ |
| Vault Support | ✅ | ❌ | ❌ |
...
- Documentation: https://h2kvm.readthedocs.io/
