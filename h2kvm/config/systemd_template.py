...
# h2kvm/config/systemd_template.py
...
def _join_rw_paths(rw: Any) -> str:
    ...
    if isinstance(rw, str) and rw.strip():
        return rw.strip()
    return "/var/lib/h2kvm /var/log/h2kvm /tmp"
...
class SystemdUnitParams:  # pylint: disable=too-many-instance-attributes  # dataclass models many independent unit-file fields
    ...
    group: str = "root"
    workdir: str = "/"
    env_file: str = "/etc/default/h2kvm"
    rw_paths: str = "/var/lib/h2kvm /var/log/h2kvm /tmp"
    extra_args: str = ""
...
def _infer_defaults(args: Any) -> SystemdUnitParams:
    ...
    script = getattr(args, "script", None) or getattr(args, "entrypoint", None) or "/path/to/h2kvm.py"
    ...
    env_file = getattr(args, "env_file", None) or "/etc/default/h2kvm"
...
def _validate_params(p: SystemdUnitParams) -> None:
    ...
    if not p.user:
        raise ValueError("Systemd unit requires a 'user' to run as (e.g., 'root' or 'h2kvm').")
    if not p.group:
        raise ValueError("Systemd unit requires a 'group' to run as (e.g., 'root' or 'h2kvm').")
    if not p.workdir:
