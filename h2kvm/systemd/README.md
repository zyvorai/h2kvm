...
Integration of systemd command-line tools into h2kvm for enhanced VM
...
```python
from h2kvm.systemd import SystemdNotify
...
```python
from h2kvm.systemd import SystemdTmpfiles
...
```python
from h2kvm.systemd import SystemdDetectVirt, VirtType
...
```python
from h2kvm.systemd import systemd_escape, systemd_unescape
...
```python
from h2kvm.systemd import SystemdMount
...
```python
from h2kvm.systemd import SystemdPath
...
import os
migration_tmp = os.path.join(temp_dir, "h2kvm")
...
```python
from h2kvm.systemd import SystemdRepart
...
vmspawn.spawn_secure_boot(
    Path("/var/lib/h2kvm/uefi-vm.qcow2"),
    memory="2G",
)
...
```python
from h2kvm.systemd import SystemdDelta
...
```python
from h2kvm.systemd import SystemdId128
...
```python
from h2kvm.pipeline.systemd_integration import SystemdPipelineIntegration
...
inhibit.run(
    ["h2kvm", "migrate", "--vm", "production-db"],
    what="idle:sleep:shutdown",
...
stats = cgtop.monitor_service("h2kvm.service", duration=60)
