# Building the hyper2kvm .deb Package
...
ls -lh ../hyper2kvm_0.3.0-1_all.deb
```
...
DEB=/tmp/hyper2kvm_0.3.0-1_all
mkdir -p $DEB/usr $DEB/DEBIAN

# 3. Install Python package
python3 -m pip install --no-index --no-deps --root $DEB --prefix /usr \
    --ignore-installed --no-build-isolation dist/hyper2kvm-0.3.0-py3-none-any.whl
...
mkdir -p $DEB/var/lib/hyper2kvm/conversions $DEB/var/log/hyper2kvm
...
Description: Production-grade hypervisor to KVM/QEMU migration toolkit
 hyper2kvm migrates virtual machines from VMware vSphere, Hyper-V, Azure,
...
#!/bin/bash
set -e
mkdir -p /var/lib/hyper2kvm/conversions /var/log/hyper2kvm
echo "hyper2kvm 0.3.0 installed. Run: sudo h2kvmctl --version"
EOF
...
sudo apt install -y ./hyper2kvm_0.3.0-1_all.deb

# Or force install (skip dependency resolution)
sudo dpkg -i hyper2kvm_0.3.0-1_all.deb
...
| `/usr/lib/python3/dist-packages/hyper2kvm/` | Python package |
| `/etc/hyper2kvm/config.yaml` | System configuration |
| `/etc/hyper2kvm/daemon.yaml` | Daemon mode configuration |
| `/etc/hyper2kvm/migrations/*.yaml` | Example migration configs |
| `/etc/modprobe.d/hyper2kvm-nbd.conf` | NBD module config |
| `/lib/systemd/system/hyper2kvm.service` | Daemon service |
| `/lib/systemd/system/hyper2kvm@.service` | Template service |
| `/usr/share/doc/hyper2kvm/examples/` | Example YAML configs |
| `/usr/libexec/hyper2kvm/install-deps.sh` | Dependency installer |
...
```bash
sudo apt remove hyper2kvm
# Or purge (remove configs too):
sudo apt purge hyper2kvm
```
