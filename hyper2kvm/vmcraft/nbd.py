...
from hyper2kvm.core.constants import DELAY_STATUS_POLL, NBD_MAX_DEVICE
    ...
    def add_allowed_directory(self, path: Path) -> None:
        ...
        try:
            ...
            self.logger.warning(
                ...
                f"    In /etc/hyper2kvm/config.yaml:\n"
                f"        allowed_dirs:\n"
...
class NBDDeviceManager:  # pylint: disable=too-many-instance-attributes  # tracks device, socket, lock, and conversion state for the full connect/disconnect lifecycle
        ...
        *,
        ...
        Args:
            ...
            use_socket: Use UNIX socket mode instead of kernel NBD
                ...
                Socket path: ``/run/hyper2kvm/nbd-{image_stem}.sock``.
        ...
        if conversion_dir:
            ...
            # Default: /var/lib/hyper2kvm/conversions (created by packaging)
            self._conversion_dir = Path("/var/lib/hyper2kvm/conversions")
        ...
        self._lock_dir = Path("/run/hyper2kvm")
        try:
            self._lock_dir.mkdir(parents=True, exist_ok=True)
        except OSError:
            # Fallback to user directory if /var/lock not writable
            self._lock_dir = Path.home() / ".cache" / "hyper2kvm" / "locks"
            self._lock_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _check_nbd_module(self) -> None:
        ...
        ``/etc/modprobe.d/hyper2kvm-nbd.conf`` which is installed by
    ...
    def find_free_nbd(self) -> str:
        ...
        raise RuntimeError(
            ...
            "  2. Clean up orphaned devices: hyper2kvm will auto-clean on next run\n"
            ...
            if check_lvm:
                ...
                if activate_lvm and report["lvm"]["volume_groups"]:
                    ...
                    try:
                        ...
                        # hyper2kvm/vmcraft/storage.py's _settle_udev --
    ...
    def _connect_via_socket(
        ...
        ``/run/hyper2kvm/nbd-{image_stem}.sock``.  The socket can be used
