...
from hyper2kvm.vmcraft.nbd import NBDDeviceManager, NBDSecurity
from hyper2kvm.vmcraft.storage import StorageStackActivator
...
def launch_backend(vm) -> None:
    ...
    with TraceContext(vm_id=primary_image, workflow="vm_activation", component="lifecycle"):
        ...
        # Finalize
        # ------------------------------------------------------------------
        vm._mount_root = Path(tempfile.mkdtemp(prefix="hyper2kvm-guestfs-"))
