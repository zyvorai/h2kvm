...
Example (direct mode):
    from hyper2kvm.vmcraft.lvm_executor import LVMExecutor, LVMJob
...
def _lvm_device_filter(nbd_device: str) -> str:
    ...
    # hyper2kvm/vmcraft/storage.py (_get_lvm_device_filter) -- structurally
...
def _ensure_nbd_module(log: logging.Logger | None = None) -> None:
    ...
    ``/etc/modprobe.d/hyper2kvm-nbd.conf`` which is installed by the
...
class _NBDPool:
    ...
    def __init__(self, max_devices: int = 32, lock_dir: Path | None = None):
        ...
        self._lock_dir = lock_dir or Path("/run/lock/hyper2kvm-nbd")
...
class _IsolatedLVMContext:  # pylint: disable=too-many-instance-attributes  # tracks its full connect/isolate/cleanup state
    ...
    def _create_isolated_env(self) -> tuple[dict[str, str], Path]:
        ...
        lvm_dir = Path(tempfile.gettempdir()) / f"hyper2kvm-lvm-{os.getpid()}-{tid}"
...
_HYPER2KVM_LVM_IMAGE = "localhost/hyper2kvm-lvm:latest"
...
def _ensure_lvm_image(log: logging.Logger, runtime: str | None = None) -> str:
    ...
    If ``localhost/hyper2kvm-lvm:latest`` is not present, it is built
    ...
    if rt == "docker":
        check_cmd = [rt, "image", "inspect", _HYPER2KVM_LVM_IMAGE]
    else:
        check_cmd = [rt, "image", "exists", _HYPER2KVM_LVM_IMAGE]
    ...
    if result.returncode == 0:
        log.debug("Container image %s already exists (%s)", _HYPER2KVM_LVM_IMAGE, rt)
        return _HYPER2KVM_LVM_IMAGE

    # Build it
    log.info(
        "Building container image %s via %s (one-time operation)...",
        _HYPER2KVM_LVM_IMAGE,
        rt,
        extra={
            "ctx": {"event": "container_image_build_start", "image": _HYPER2KVM_LVM_IMAGE, "runtime": rt}
        },
    )
    containerfile = (
    ...
    build = subprocess.run(
        [rt, "build", "--network=host", "-t", _HYPER2KVM_LVM_IMAGE, "-f", "-"],
        input=containerfile,
        capture_output=True,
        text=True,
        timeout=600,
    ...
    if build.returncode != 0:
        raise RuntimeError(f"Failed to build {_HYPER2KVM_LVM_IMAGE} via {rt}: {build.stderr.strip()}")
    log.info(
        "Built container image %s via %s",
        _HYPER2KVM_LVM_IMAGE,
        rt,
        extra={
            "ctx": {"event": "container_image_build_complete", "image": _HYPER2KVM_LVM_IMAGE, "runtime": rt}
        },
    )
    return _HYPER2KVM_LVM_IMAGE
...
class _PodmanWorker:  # pylint: disable=too-few-public-methods  # single-purpose worker, run() is its only operation
    ...
    On first use, builds ``localhost/hyper2kvm-lvm:latest`` from
    ...
    def __init__(
        ...
        # Otherwise build/reuse the hyper2kvm-lvm image with LVM tools.
...
class LVMExecutor:  # pylint: disable=too-many-instance-attributes  # holds full pool/job/lock/timeout runtime state
    ...
    Args:
        ...
        podman_image: Container image for Podman mode.  Pass ``"auto"``
            to auto-build and cache ``localhost/hyper2kvm-lvm:latest``
