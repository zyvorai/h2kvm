...
"""
Argument group definitions for hyper2kvm CLI.
...
from hyper2kvm import __version__
from hyper2kvm.core.constants import DEFAULT_CONTAINER_ISOLATION, SIZE_1_MIB
...
def _add_fixing_behavior(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        "If not specified, auto-discovered at /var/lib/hyper2kvm/virtio-win.iso "
...
def _add_luks_knobs(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        help="Passphrase for LUKS-encrypted disks (or set HYPER2KVM_LUKS_PASSPHRASE "
...
def _add_domain_emission(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        ...
        "Drivers are copied to C:\\hyper2kvm-drivers and installed via pnputil on first boot.",
    )
    p.add_argument(
        "--win-localtime-clock",
...
def _add_zkvm_flags(p: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        ...
        "(default: /run/hyper2kvm/zkvm.sock for root, "
        "$XDG_RUNTIME_DIR/hyper2kvm/zkvm.sock for users).",
    )
