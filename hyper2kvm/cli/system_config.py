...
"""System-wide configuration loader for hyper2kvm."""
...
USER_CONFIG_PATHS = [
    Path(_xdg_config) / "hyper2kvm" / "config.yaml",
    Path(_xdg_config) / "hyper2kvm" / "config.yml",
    Path.home() / ".hyper2kvm.yaml",
]

# System-wide configuration locations (in order of precedence)
SYSTEM_CONFIG_PATHS = [
    Path("/etc/hyper2kvm/config.yaml"),
    Path("/etc/hyper2kvm/config.yml"),
    Path("/usr/local/etc/hyper2kvm/config.yaml"),
]


def load_system_config(logger: logging.Logger | None = None) -> dict[str, Any]:
    """
    Load system-wide configuration from /etc/hyper2kvm/config.yaml.
