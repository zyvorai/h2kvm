...
# chain), the top-level hyper2kvm package is already fully initialized in
...
if TYPE_CHECKING:
    ...
    from hyper2kvm.core.validation_suite import ValidationSuite
...
class OfflineFSFix:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def open(self) -> guestfs.GuestFS:
        ...
        if backend == "vmcraft" and (self.luks_enable or self._quick_probe_lvm_luks()):
            ...
            from hyper2kvm.core.guestfs_factory import _guestfs_appliance_available
            ...
            if _guestfs_appliance_available():
                ...
                self.logger.warning(
                    ...
                    "Re-run `hyper2kvm doctor` or `libguestfs-test-tool` after installing."
                )
        ...
        try:
            ...
            if modified:
                ...
                self.logger.warning(
                    ...
                    "    hyper2kvm will attempt an automatic fstab rebuild next.\n"
    ...
    def _configure_auto_grow(self, g: guestfs.GuestFS, root_fstype) -> tuple[dict[str, str], list[str]]:
        ...
        try:
            # pylint: disable=import-outside-toplevel  # lazy import: optional dependency or avoids a circular import
            from hyper2kvm.infrastructure.systemd.boot import FilesystemType
...
"""
    ...
    def _configure_tmpfiles(self, g: guestfs.GuestFS) -> tuple[dict[str, str], list[str]]:
        ...
        try:
            ...
            tmpfiles_conf = f"{tmpfiles_dir}/hyper2kvm.conf"
            tmpfiles_content = """# hyper2kvm tmpfiles configuration
d /var/lib/hyper2kvm 0755 root root -
d /var/log/hyper2kvm 0755 root root -
d /run/hyper2kvm 0755 root root -
"""
            g.write(tmpfiles_conf, tmpfiles_content)
            features["tmpfiles"] = "configured"
            self.logger.info("✓ Created hyper2kvm tmpfiles configuration")
        # pylint: disable=broad-exception-caught  # best-effort fixer step, must not abort the whole migration
        except Exception as e:
...
"""
    ...
    def _apply_firstboot_integration(
        ...
        try:
            # pylint: disable=import-outside-toplevel  # lazy import: optional dependency or avoids a circular import
            from hyper2kvm.infrastructure.systemd.firstboot import (
                FirstbootConfig,
            ...
            try:
                ...
                g.write(f"{generator_dir}/hyper2kvm-generator", generator_content)
                g.chmod(0o755, f"{generator_dir}/hyper2kvm-generator")
                firstboot_result["generator_created"] = True
                self.logger.info("Created systemd generator: hyper2kvm-generator")
            # pylint: disable=broad-exception-caught  # best-effort fixer step, must not abort the whole migration
            except Exception as e:
            ...
            try:
                ...
                g.write(f"{service_dir}/hyper2kvm-firstboot.service", service_content)
                g.chmod(0o644, f"{service_dir}/hyper2kvm-firstboot.service")
                firstboot_result["service_created"] = True
                self.logger.info("Created firstboot service: hyper2kvm-firstboot.service")
            # pylint: disable=broad-exception-caught  # best-effort fixer step, must not abort the whole migration
            except Exception as e:
            ...
            try:
                ...
                g.write(f"{libexec_dir}/hyper2kvm-firstboot", script_content)
                g.chmod(0o755, f"{libexec_dir}/hyper2kvm-firstboot")
                firstboot_result["script_created"] = True
                self.logger.info("Created firstboot script: hyper2kvm-firstboot")
            # pylint: disable=broad-exception-caught  # best-effort fixer step, must not abort the whole migration
            except Exception as e:
            ...
            try:
                if not g.is_dir("/etc/hyper2kvm"):
                    g.mkdir_p("/etc/hyper2kvm")
                g.write("/etc/hyper2kvm/converted", "Converted by Hyper2KVM\n")
...
"""Hyper2KVM systemd generator - enables firstboot service if conversion detected"""
import os, sys

def main():
    if not os.path.exists("/etc/hyper2kvm/converted"):
        sys.exit(0)
    wants_dir = os.path.join("/run/systemd/system", "multi-user.target.wants")
    os.makedirs(wants_dir, exist_ok=True)
    symlink = os.path.join(wants_dir, "hyper2kvm-firstboot.service")
    if not os.path.exists(symlink):
        try:
            os.symlink("/usr/lib/systemd/system/hyper2kvm-firstboot.service", symlink)
...
"""
    ...
    def apply_systemd_boot_integration(self, g: guestfs.GuestFS) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                # pylint: disable=import-outside-toplevel  # lazy import: optional dependency or avoids a circular import
                from hyper2kvm.infrastructure.systemd.boot import (
                    BootEnvironment,
                    BootType,
                    FilesystemType,
                )
    ...
    def _apply_migration_hardening(self, g: Any) -> dict[str, Any]:
        ...
        def _blacklist_vmware_modules() -> None:
            content = (
                ...
                "# Written by hyper2kvm migration\n"
        ...
        def _remove_ssh_host_keys() -> None:
            try:
                ...
                if not has_regen:
                    for path in ["/usr/bin/cloud-init", "/etc/hyper2kvm/firstboot.sh"]:
                        try:
                            if g.is_file(path):
                                has_regen = True
                                break
        ...
        def _check_firewalld_zones() -> None:
            try:
                if not g.is_dir("/etc/firewalld/zones"):
                    return
                # pylint: disable=import-outside-toplevel  # lazy import: optional dependency or avoids a circular import
                from hyper2kvm.fixers.network.topology import INTERFACE_NAME_PATTERNS
        ...
        if is_win:
            ...
            try:
                ...
                if not self.detected_windows_build:
                    try:
                        # pylint: disable=import-outside-toplevel  # lazy import: optional dependency or avoids a circular import
                        from hyper2kvm.fixers.windows.virtio.detection import (
