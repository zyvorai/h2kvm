...
VIRTIO_RUNONCE_CMD = r"C:\Windows\Temp\hyper2kvm-virtio-install.bat"

# Batch file content staged into the guest filesystem
VIRTIO_INSTALL_BAT = r"""@echo off
setlocal

echo [hyper2kvm] Starting VirtIO driver installation...
...
echo [hyper2kvm] OS=%OSFOLDER% ARCH=%ARCH%
...
echo [hyper2kvm] VirtIO ISO not found
goto :done

:found
echo [hyper2kvm] Found VirtIO ISO at %ISO%
...
for %%c in (viostor NetKVM Balloon vioserial viorng qxldod viogpudo vioinput viofs pvpanic smbus qemufwcfg) do (
    if exist "%ISO%\%%c\%OSFOLDER%\%ARCH%" (
        echo [hyper2kvm] Installing %%c from %OSFOLDER%\%ARCH%
...
if exist "%ISO%\guest-agent\qemu-ga-x86_64.msi" (
    echo [hyper2kvm] Installing QEMU guest agent...
    msiexec /i "%ISO%\guest-agent\qemu-ga-x86_64.msi" /quiet /norestart
)

echo [hyper2kvm] VirtIO driver installation completed

:done
del "%~f0"
"""

# Custom PnP pnputil command — installs from C:\hyper2kvm-drivers staging dir
CUSTOM_PNP_RUNONCE_CMD = (
    'cmd.exe /c "if exist C:\\hyper2kvm-drivers'
    ' pnputil /add-driver C:\\hyper2kvm-drivers\\*.inf /subdirs /install"'
)
...
class WindowsVirtioDeployer:  # pylint: disable=too-few-public-methods  # deliberately exposes only prepare_offline(); rest is private orchestration
    ...
    Performs offline registry edits via ntfs-3g + hivex:
      ...
      - Stages custom PnP drivers into C:\\hyper2kvm-drivers
    ...
    def _stage_custom_drivers(self, mount_point: Path) -> list[str]:
        ...
        Drivers are staged at C:\\hyper2kvm-drivers so pnputil can find them
        ...
        staged = []
        stage_dir = mount_point / "hyper2kvm-drivers"
        ...
        for driver_path in self.custom_pnp_drivers:
            ...
            if driver_path.suffix.lower() == ".iso":
                ...
                iso_mount = Path(tempfile.mkdtemp(prefix="hyper2kvm-iso-"))
                try:
                    subprocess.run(
    ...
    def _mount_ntfs(self, device: str) -> Path:
        ...
        mount_point = Path(tempfile.mkdtemp(prefix="hyper2kvm-win-"))
        subprocess.run(
