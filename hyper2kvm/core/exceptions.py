...
"""
Comprehensive exception hierarchy for hyper2kvm.

Provides subsystem-specific exceptions with context, redaction, and user-friendly
error messages. All exceptions inherit from Hyper2KvmError base class.
...
class ErrorCode(enum.Enum):
    """
    Machine-readable error codes for hyper2kvm operations.
...
_ERROR_HINTS: dict[ErrorCode, str] = {
    ...
    ErrorCode.LUKS_KEY_MISSING: "Provide the LUKS passphrase via --luks-key or HYPER2KVM_LUKS_KEY env var.",
...
def create_helpful_error(  # pylint: disable=too-many-arguments
    ...
    **context: Any,
    ...
    if doc_link:
        context["doc_link"] = f"https://github.com/ssahani/hyper2kvm/blob/main/docs/{doc_link}"
...
class ProviderError(Hyper2KvmError):
...
class FixerError(Hyper2KvmError):
...
class StorageError(Hyper2KvmError):
...
class ValidationError(Hyper2KvmError):
...
class Hyper2KvmRuntimeError(Hyper2KvmError):
    """Base class for runtime execution errors (daemon, worker, operator)."""


class DaemonError(Hyper2KvmRuntimeError):
    """Daemon operation failed."""


class WorkerError(Hyper2KvmRuntimeError):
    """Worker execution failed."""


class OperatorError(Hyper2KvmRuntimeError):
...
class HookExecutionError(Hyper2KvmRuntimeError):
...
class VMCraftError(Hyper2KvmError):
...
class InfrastructureError(Hyper2KvmError):
...
class CommandError(Hyper2KvmError):
