...
"""
Hyper2KVM LUKS CLI Tool

Command-line interface for LUKS auto-unlock operations.

Usage:
    hyper2kvm-luks unlock               # Unlock all LUKS devices
    hyper2kvm-luks seal <device> <key>  # Seal key to TPM2
    hyper2kvm-luks list                 # List LUKS devices
    hyper2kvm-luks status               # Show unlock status
"""
...
def load_config(config_path: Optional[str] = None) -> dict:
    ...
    if config_path and Path(config_path).exists():
        with open(config_path, encoding="utf-8") as f:
            config = json.load(f)
    elif Path("/etc/hyper2kvm/luks.json").exists():
        with open("/etc/hyper2kvm/luks.json", encoding="utf-8") as f:
            config = json.load(f)

    # Override with environment variables
    if os.getenv("HYPER2KVM_TPM_HANDLE"):
        config["tpm_handle"] = os.getenv("HYPER2KVM_TPM_HANDLE")

    if os.getenv("HYPER2KVM_VAULT_ADDR"):
        config["vault"] = {
            "addr": os.getenv("HYPER2KVM_VAULT_ADDR"),
            "token": os.getenv("HYPER2KVM_VAULT_TOKEN"),
            "path": os.getenv("HYPER2KVM_VAULT_PATH", "secret/hyper2kvm/luks"),
        }

    if os.getenv("HYPER2KVM_LUKS_KEYFILE"):
        config["keyfile"] = os.getenv("HYPER2KVM_LUKS_KEYFILE")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Hyper2KVM LUKS Auto-Unlock Tool")

    parser.add_argument("-v", "--verbose", action="store_true", help="Enable verbose logging")

    parser.add_argument("-c", "--config", help="Config file path (default: /etc/hyper2kvm/luks.json)")
