...
from hyper2kvm.core.exceptions import Fatal, Hyper2KvmError, create_helpful_error
...
from hyper2kvm.core.sanity_checker import SanityChecker
from hyper2kvm.core.structured_log import PhaseTimer, log_event
from hyper2kvm.core.utils import U
from hyper2kvm.libvirt.domain_emitter import emit_from_args
from hyper2kvm.providers.vmware.vsphere.mode import VsphereMode
from hyper2kvm.quality.testing.libvirt_tester import LibvirtTest
from hyper2kvm.quality.testing.qemu_tester import QemuTest
...
# Check availability
try:
    from hyper2kvm.providers.vmware.clients.client import PYVMOMI_AVAILABLE
except ImportError:
    PYVMOMI_AVAILABLE = False

try:
    from hyper2kvm.providers.vmware.transports.http_client import REQUESTS_AVAILABLE
except ImportError:
...
class Orchestrator:  # pylint: disable=too-many-instance-attributes  # coordinates many independent handler components across the migration pipeline
        ...
        if not self.disk_processor:
            raise Fatal(
                ...
                msg=(
                    ...
                    "https://github.com/ssahani/hyper2kvm/issues with your config and logs."
                ),
            )
    ...
    def _deploy_to_kubernetes(self, out_images: list[Path]) -> None:
        ...
        try:
            ...
            from hyper2kvm.infrastructure.deployers.kubernetes import deploy_to_kubernetes
            ...
            for img in out_images:
                if str(img).endswith(".qcow2"):
                    try:
                        ...
                        err_text = (
                            e.user_message(include_context=True) if isinstance(e, Hyper2KvmError) else str(e)
                        )
                        ...
                        if not isinstance(e, Hyper2KvmError):
                            self.logger.exception(
            ...
            raise Fatal(
                ...
                msg=(
                    ...
                    "Or install with KubeVirt support: pip install hyper2kvm[kubernetes]"
                ),
            ) from e
        ...
        try:
            ...
            raise Fatal(
                ...
                msg=(
                    ...
                    "Install with: pip install 'hyper2kvm[openstack]'"
                ),
            ) from e
    ...
    _DAEMON_MODES = [
        ...
        (
            ...
            "hyper2kvm.runtime.daemon.workflow_daemon",
            "WorkflowDaemon",
        ),
    ]
    ...
    def _run_daemon_mode(self) -> None:
        ...
        from hyper2kvm.runtime.daemon.daemon_watcher import (  # pylint: disable=import-outside-toplevel  # daemon subsystem only needed when this mode is selected
            DaemonWatcher,
        )
    ...
    def _apply_windows_virtio_deploy(self, out_images: list[Path]) -> None:
        ...
        try:
            ...
            from hyper2kvm.fixers.windows.virtio_deploy import WindowsVirtioDeployer
    ...
    def _run_virtio_staged_boot(self, out_root: Path, out_images: list[Path]) -> None:
        ...
        try:
            ...
            from hyper2kvm.fixers.windows.virtio_stage import VirtioStagedDeployer
