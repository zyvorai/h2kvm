...
# Filesystem access
ReadWritePaths=/var/lib/libvirt/images /mnt/vmroot /var/log/hyper2kvm
ReadOnlyPaths=/usr /etc

# Logging
StandardOutput=journal
StandardError=journal
SyslogIdentifier=hyper2kvm-{name}

[Install]
WantedBy=multi-user.target
"""

        service_path = Path(f"/etc/systemd/system/hyper2kvm-{name}.service")
...
Requires=hyper2kvm-{name}.service
...
"""
        ...
        timer_path = Path(f"/etc/systemd/system/hyper2kvm-{name}.timer")
        timer_path.write_text(timer_content)

        # Enable and start timer
        subprocess.run(["systemctl", "enable", f"hyper2kvm-{name}.timer"], check=False)
        subprocess.run(["systemctl", "start", f"hyper2kvm-{name}.timer"], check=False)
    ...
    def create_transient_scope(self, pid: int, name: str, cpu_quota: str = "50%", memory_max: str = "2G"):
        ...
        try:
            scope_name = f"hyper2kvm-{name}-{pid}.scope"

            properties = [
                ("Description", f"Hyper2KVM Process {name}"),
                ("CPUQuota", cpu_quota),
