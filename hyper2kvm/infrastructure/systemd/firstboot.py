...
Production-grade systemd-firstboot integration for Hyper2KVM converted images.
...
class SystemdFirstboot:
    """
    Systemd firstboot integration for Hyper2KVM.
    ...
    CONVERSION_FLAG = Path("/etc/hyper2kvm/converted")
    ...
    def create_generator(self) -> Path:
        ...
        """
        generator_path = self.chroot / self.GENERATOR_DIR.relative_to("/") / "hyper2kvm-generator"
...
"""
    ...
    def create_firstboot_service(self) -> Path:
        ...
        """
        service_path = self.chroot / self.SYSTEM_DIR.relative_to("/") / "hyper2kvm-firstboot.service"
...
"""
    ...
    def create_firstboot_script(self, config: Optional[FirstbootConfig] = None) -> Path:
        ...
        script_path = self.chroot / self.LIBEXEC_DIR.relative_to("/") / "hyper2kvm-firstboot"
    ...
    def mark_for_firstboot(self) -> dict[str, Any]:
        ...
        try:
            ...
            flag_path.write_text("Converted by Hyper2KVM\n")
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        if all(report["components_installed"].values()) and report["firstboot_marked"]:
            report["expected_journal_output"] = [
                "systemd[1]: Starting Hyper2KVM First Boot Initialization...",
                "hyper2kvm-firstboot[NNN]: Hyper2KVM First Boot Initialization Started",
                "hyper2kvm-firstboot[NNN]: Step 1/6: Generating new machine-id",
                "hyper2kvm-firstboot[NNN]: Step 2/6: Regenerating initramfs",
                "hyper2kvm-firstboot[NNN]: Step 3/6: Regenerating GRUB configuration",
                "hyper2kvm-firstboot[NNN]: Step 4/6: Activating LVM volumes",
                "hyper2kvm-firstboot[NNN]: Step 5/6: Settling udev",
                "hyper2kvm-firstboot[NNN]: Step 6/6: Reloading systemd daemon",
                "hyper2kvm-firstboot[NNN]: Hyper2KVM First Boot Initialization Completed",
                "systemd[1]: Finished Hyper2KVM First Boot Initialization",
            ]
