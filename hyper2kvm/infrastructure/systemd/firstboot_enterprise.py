...
def generate_enterprise_firstboot_script(config: Optional[FirstbootConfig] = None) -> str:
    ...
    script_lines = [
        ...
        "",
        "#" + "=" * 78,
        "# Hyper2KVM Enterprise First Boot Initialization",
        ...
        "",
        'log "=" + "=" * 76',
        'log "Hyper2KVM Enterprise First Boot Initialization Started"',
        'log "=" + "=" * 76',
        "",
        "# Detect hypervisor",
    ...
    if config.regenerate_machine_id:
        script_lines.extend(
            [
                ...
                "systemd-machine-id-setup 2>&1 | systemd-cat -t hyper2kvm-firstboot",
    ...
    if config.trigger_hardware_detection:
        script_lines.extend(
            [
                ...
                "udevadm trigger --action=add 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "udevadm settle 2>&1 | systemd-cat -t hyper2kvm-firstboot",
    ...
    if config.inject_virtio_drivers:
        script_lines.extend(
            [
                ...
                "    dracut -f --add-drivers 'virtio virtio_blk virtio_scsi virtio_net virtio_pci' 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                ...
                "    update-initramfs -u 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "fi",
                "depmod -a",
    ...
    if config.regenerate_network:
        script_lines.extend(
            [
                ...
                "    nmcli connection reload 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "    # Reapply all connections",
                "    for conn in $(nmcli -t -f NAME connection show); do",
                '        nmcli device reapply "$conn" 2>&1 | systemd-cat -t hyper2kvm-firstboot || true',
                "    done",
    ...
    if config.fix_disk_uuids:
        script_lines.extend(
            [
                ...
                "if [ -f /etc/fstab ]; then",
                "    cp /etc/fstab /etc/fstab.pre-hyper2kvm",
                '    log "  Backed up fstab to /etc/fstab.pre-hyper2kvm"',
    ...
    if config.activate_lvm:
        script_lines.extend(
            [
                ...
                "    pvscan --cache 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    vgscan --mknodes 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    vgchange -ay 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    lvscan 2>&1 | systemd-cat -t hyper2kvm-firstboot",
    ...
    if config.reinstall_grub:
        script_lines.extend(
            [
                ...
                "        grub2-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=BOOT 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "        grub2-mkconfig -o /boot/efi/EFI/*/grub.cfg 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    elif command -v grub-install >/dev/null 2>&1; then",
                "        grub-install --target=x86_64-efi --efi-directory=/boot/efi --bootloader-id=BOOT 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "        update-grub 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    fi",
                "else",
                '    log "  Reinstalling GRUB (BIOS mode)"',
                "    if command -v grub2-install >/dev/null 2>&1; then",
                '        grub2-install "$BOOT_DEV" 2>&1 | systemd-cat -t hyper2kvm-firstboot',
                "        grub2-mkconfig -o /boot/grub2/grub.cfg 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    elif command -v grub-install >/dev/null 2>&1; then",
                '        grub-install "$BOOT_DEV" 2>&1 | systemd-cat -t hyper2kvm-firstboot',
                "        update-grub 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "    fi",
                "fi",
    ...
    if config.install_qemu_guest_agent:
        script_lines.extend(
            [
                ...
                "        dnf install -y qemu-guest-agent 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "    elif command -v yum >/dev/null 2>&1; then",
                "        yum install -y qemu-guest-agent 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "    elif command -v apt-get >/dev/null 2>&1; then",
                "        apt-get update -qq 2>&1 | systemd-cat -t hyper2kvm-firstboot",
                "        apt-get install -y qemu-guest-agent 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "    fi",
                "    systemctl enable --now qemu-guest-agent 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
    ...
    if config.regenerate_ssh_keys:
        script_lines.extend(
            [
                ...
                "# Generate new keys",
                "ssh-keygen -A 2>&1 | systemd-cat -t hyper2kvm-firstboot",
    ...
    if config.apply_virtual_guest_tuning:
        script_lines.extend(
            [
                ...
                "    tuned-adm profile virtual-guest 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
    ...
    if config.enable_cloud_init:
        script_lines.extend(
            [
                ...
                "    systemctl enable cloud-init cloud-config cloud-final cloud-init-local 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                "    # Create hyper2kvm cloud-init datasource config",
                "    mkdir -p /etc/cloud/cloud.cfg.d",
                "    cat > /etc/cloud/cloud.cfg.d/99-hyper2kvm.cfg <<'CLOUDEOF'",
                "# Hyper2KVM cloud-init configuration",
                "datasource_list: [ NoCloud, None ]",
                "CLOUDEOF",
                'log "  ✓ cloud-init enabled with hyper2kvm config"',
                "else",
    ...
    if config.create_conversion_metadata:
        script_lines.extend(
            [
                ...
                "cat > /etc/hyper2kvm/metadata.json <<METAEOF",
                "{",
                ...
                "}",
                "METAEOF",
                'log "  ✓ Conversion metadata saved to /etc/hyper2kvm/metadata.json"',
                "STEP=$((STEP+1))",
                "",
            ]
        )
    ...
    script_lines.extend(
        [
            ...
            "systemctl daemon-reexec 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
        ...
        for i, cmd in enumerate(config.custom_commands, 1):
            script_lines.extend(
                [
                    ...
                    f"{cmd} 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
                ]
            )
        script_lines.extend(
            [
    ...
    script_lines.extend(
        [
            ...
            "systemctl disable hyper2kvm-firstboot.service 2>&1 | systemd-cat -t hyper2kvm-firstboot || true",
            'log "  ✓ Conversion flag removed, service disabled"',
            "",
            'log "=" + "=" * 76',
            'log "Hyper2KVM Enterprise First Boot Initialization Completed Successfully"',
            'log "=" + "=" * 76',
            "",
            "# Log boot metrics",
