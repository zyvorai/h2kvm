...
This package contains supporting infrastructure services used across hyper2kvm.
...
**Example:**
```python
from hyper2kvm.infrastructure.systemd import SystemdManager
...
**Example:**
```python
from hyper2kvm.infrastructure.ssh import inject_ssh_keys
...
hooks:
  post-fixing:
    - type: script
      ...
      timeout: 300
    - type: ansible
      playbook: /etc/hyper2kvm/playbooks/setup.yml
```

**Example:**
```python
from hyper2kvm.infrastructure.hooks import HookRunner, HookType
...
**Example:**
```python
from hyper2kvm.infrastructure.rollback import SnapshotManager
...
**Example:**
```python
from hyper2kvm.infrastructure.deployers import LibvirtDeployer
...
```python
from hyper2kvm.infrastructure.systemd.boot import create_firstboot_service
...
from hyper2kvm.infrastructure.hooks import HookRunner
...
```python
from hyper2kvm.infrastructure.rollback import with_rollback
...
```yaml
# /etc/hyper2kvm/systemd.yaml
systemd:
  firstboot_template: |
    [Unit]
    Description={{ description }}
    After=network.target
    ConditionPathExists=!/var/lib/hyper2kvm/firstboot-complete

    [Service]
    Type=oneshot
    ExecStart={{ exec_start }}
    ExecStartPost=/usr/bin/touch /var/lib/hyper2kvm/firstboot-complete
...
- [x] OpenStack Glance deploy (`--deploy-openstack`, `hyper2kvm/infrastructure/deployers/openstack.py`)
