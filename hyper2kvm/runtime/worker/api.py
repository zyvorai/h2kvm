...
"""
REST API for hyper2kvm Worker Job Protocol v1.
...
Usage:
    ...
    # Production
    gunicorn hyper2kvm.worker.api:app -w 4 -k uvicorn.workers.UvicornWorker

    # Docker
    docker run -p 8000:8000 hyper2kvm/worker-api:latest
...
_TOKEN_FILE = Path("/var/run/secrets/hyper2kvm/token")
...
def _load_api_token() -> str | None:
    ...
    Resolution order:
        1. HYPER2KVM_API_TOKEN environment variable
        2. /var/run/secrets/hyper2kvm/token file
        3. None (auth disabled — backward compatible)
    """
    token = os.environ.get("HYPER2KVM_API_TOKEN")
    if token:
        return token.strip()
...
API_VERSION = "v1"
API_TITLE = "hyper2kvm Worker Job Protocol API"
API_DESCRIPTION = """
...
async def lifespan(app: FastAPI):
    ...
    """
    logger.info("Starting hyper2kvm Worker API...")
    ...
    # Shutdown: Save state
    logger.info("Shutting down hyper2kvm Worker API...")
    cleanup_task.cancel()
