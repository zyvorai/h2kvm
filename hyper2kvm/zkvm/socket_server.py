...
# hyper2kvm/tui/socket_server.py
...
_ROOT_SOCKET_DIR = "/run/hyper2kvm"
# Socket filename
...
def get_default_socket_path() -> str:
    ...
    For root (uid 0): /run/hyper2kvm/zkvm.sock
    For regular users: $XDG_RUNTIME_DIR/hyper2kvm/zkvm.sock

    If XDG_RUNTIME_DIR is not set for a non-root user, falls back to
    /tmp/hyper2kvm-<uid>/zkvm.sock.
    ...
    if runtime_dir:
        return os.path.join(runtime_dir, "hyper2kvm", _SOCKET_FILENAME)

    # Fallback for users without XDG_RUNTIME_DIR
    return os.path.join(f"/tmp/hyper2kvm-{os.getuid()}", _SOCKET_FILENAME)
...
class TUISocketServer:  # pylint: disable=too-many-instance-attributes  # holds full server runtime state
    """
    Asyncio Unix domain socket server for the hyper2kvm TUI bridge.
    ...
    async def _handle_list_vms(self, request: dict) -> dict:
        ...
        if source == "local":
            scan_path = request.get("path", "/tmp/hyper2kvm-output")
    ...
    async def _handle_list_local_disks(self, request: dict) -> dict:
        ...
        default_dir = settings.get("general", {}).get("default_output_dir", "/tmp/hyper2kvm-output")
    ...
    async def _handle_start_migration(self, request: dict) -> dict:
        ...
        settings = load_tui_settings(logger=self.logger)
        default_output = settings.get("general", {}).get("default_output_dir", "/tmp/hyper2kvm-output")
    ...
    async def _handle_set_config(self, request: dict) -> dict:
        ...
        if key is not None and value is not None:
            ...
            if not success:
                raise RuntimeError(
                    "Failed to save configuration. Check that ~/.config/hyper2kvm/ exists and is writable."
                )
    ...
    async def _handle_get_ai_info(self, request: dict) -> dict:  # pylint: disable=unused-argument  # fixed handler signature dispatched via REQUEST_HANDLERS
        ...
        try:
            from hyper2kvm.ai import get_ai_status  # type: ignore[import-untyped]

            ai_info = get_ai_status()
            ai_info["available"] = True
        except ImportError:
            ai_info["message"] = "AI module not installed.  Install with: pip install 'hyper2kvm[ai]'"
