...
# Bump H2KVM_INSTALL_BUNDLE_ID when default URLs or pins change.

: "${H2KVM_INSTALL_BUNDLE_ID:=2026.05.07-hivex-deploy}"

# govc: prefix directory on GitHub releases (latest by default).
: "${H2KVM_GOVC_DOWNLOAD_PREFIX:=https://github.com/vmware/govmomi/releases/latest/download}"

# virtio-win stable ISO (Fedora virtio-win stable stream).
: "${H2KVM_VIRTIO_WIN_ISO_URL:=https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/virtio-win.iso}"

h2kvm_govc_download_url() {
    local arch="$1"
    printf '%s/govc_Linux_%s.tar.gz\n' "${H2KVM_GOVC_DOWNLOAD_PREFIX}" "$arch"
}

# Fallback when distro python3.xx-hivex RPM/deb is missing: build C bindings from source.
# Bump H2KVM_INSTALL_BUNDLE_ID when this pin changes.
