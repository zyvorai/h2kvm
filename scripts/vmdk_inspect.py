...
def generate_fix_config(vmdk_path: Path, output_path: str, boot_mode: str = "BIOS") -> str:
    """Generate h2kvm YAML config to fix controller mismatch."""
...
def apply_auto_fix(info: VMDKInfo, output_path: str) -> bool:
    """
    Apply automatic fix for controller mismatch using h2kvm.
    ...
    # Check if h2kvm is available
    try:
        result = subprocess.run(["h2kvm", "--version"], capture_output=True, text=True)
        if result.returncode != 0:
            print(f"\n❌ h2kvm not found in PATH")
            print(f"   Install with: pip install h2kvm")
            print(f"\n   Manual fix: h2kvm --config {config_file}")
            return False
    except FileNotFoundError:
        print(f"\n❌ h2kvm not found in PATH")
        print(f"   Install with: pip install h2kvm")
    ...
    # Execute fix
    print(f"\n🚀 Executing fix with h2kvm...")
    print(f"   (This may require sudo for guest filesystem operations)\n")

    try:
        result = subprocess.run(["h2kvm", "--config", config_file], text=True, capture_output=True)
        ...
        if result.returncode == 0:
            ...
            if result.stderr and "requires root" in result.stderr:
                print(f"\n💡 Try with sudo:")
                print(f"   sudo h2kvm --config {config_file}")
            else:
                print(f"   Check logs above for details")
                print(f"\n💡 Manual retry:")
                print(f"   h2kvm --config {config_file}")
