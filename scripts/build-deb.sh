#!/bin/bash
# build-deb.sh — Build hyper2kvm .deb package for Debian/Ubuntu
#
# Usage:
...
#
# Output: dist/hyper2kvm_<version>-1_all.deb
...
DEB_NAME="hyper2kvm_${VERSION}-1_all"
CONTAINER_IMAGE="${1:-}"

echo "=== hyper2kvm .deb builder ==="
    ...
    $RUNTIME run --rm \
        ...
        "$CONTAINER_IMAGE" bash -c "
            ...
            python3 -m pip install --no-index --no-deps --root \$DEB --prefix /usr \
                ...
                dist/hyper2kvm-${VERSION}-py3-none-any.whl 2>&1 | tail -1

            # Configs
            mkdir -p \$DEB/etc/hyper2kvm/migrations
            cp etc/hyper2kvm/config.yaml.sample \$DEB/etc/hyper2kvm/config.yaml
            cp etc/hyper2kvm/daemon.yaml.sample \$DEB/etc/hyper2kvm/daemon.yaml
            cp etc/hyper2kvm/README.md \$DEB/etc/hyper2kvm/
            cp etc/hyper2kvm/migrations/*.yaml \$DEB/etc/hyper2kvm/migrations/

            mkdir -p \$DEB/etc/modprobe.d \$DEB/etc/sysctl.d \$DEB/etc/systemd/system.conf.d
            cp etc/modprobe.d/nbd.conf \$DEB/etc/modprobe.d/hyper2kvm-nbd.conf
            cp etc/sysctl.d/99-hyper2kvm-nbd.conf \$DEB/etc/sysctl.d/
            cp etc/systemd/system.conf.d/hyper2kvm-limits.conf \$DEB/etc/systemd/system.conf.d/

            # Systemd + docs
            mkdir -p \$DEB/lib/systemd/system
            cp systemd/hyper2kvm.service \$DEB/lib/systemd/system/
            cp systemd/hyper2kvm@.service \$DEB/lib/systemd/system/
            mkdir -p \$DEB/usr/share/doc/hyper2kvm/examples
            cp test-confs/*.yaml \$DEB/usr/share/doc/hyper2kvm/examples/
            mkdir -p \$DEB/usr/libexec/hyper2kvm
            cp scripts/install-deps.sh \$DEB/usr/libexec/hyper2kvm/
            chmod 755 \$DEB/usr/libexec/hyper2kvm/install-deps.sh
            mkdir -p \$DEB/var/lib/hyper2kvm \$DEB/var/log/hyper2kvm
...
# Install configs
mkdir -p "$DEB/etc/hyper2kvm/migrations"
install -m 644 etc/hyper2kvm/config.yaml.sample "$DEB/etc/hyper2kvm/config.yaml"
install -m 644 etc/hyper2kvm/daemon.yaml.sample "$DEB/etc/hyper2kvm/daemon.yaml"
install -m 644 etc/hyper2kvm/README.md "$DEB/etc/hyper2kvm/"
install -m 644 etc/hyper2kvm/migrations/*.yaml "$DEB/etc/hyper2kvm/migrations/"
mkdir -p "$DEB/etc/modprobe.d" "$DEB/etc/sysctl.d" "$DEB/etc/systemd/system.conf.d"
install -m 644 etc/modprobe.d/nbd.conf "$DEB/etc/modprobe.d/hyper2kvm-nbd.conf"
install -m 644 etc/sysctl.d/99-hyper2kvm-nbd.conf "$DEB/etc/sysctl.d/"
install -m 644 etc/systemd/system.conf.d/hyper2kvm-limits.conf "$DEB/etc/systemd/system.conf.d/"

# Systemd + docs
mkdir -p "$DEB/lib/systemd/system"
install -m 644 systemd/hyper2kvm.service "$DEB/lib/systemd/system/"
install -m 644 systemd/hyper2kvm@.service "$DEB/lib/systemd/system/"
mkdir -p "$DEB/usr/share/doc/hyper2kvm/examples"
install -m 644 test-confs/*.yaml "$DEB/usr/share/doc/hyper2kvm/examples/"
mkdir -p "$DEB/usr/libexec/hyper2kvm"
install -m 755 scripts/install-deps.sh "$DEB/usr/libexec/hyper2kvm/"
mkdir -p "$DEB/var/lib/hyper2kvm" "$DEB/var/log/hyper2kvm"
...
printf '#!/bin/bash\nset -e\nmkdir -p /var/lib/hyper2kvm /var/log/hyper2kvm\necho "hyper2kvm %s installed. Run: sudo h2kvmctl --version"\n' "$VERSION" > "$DEB/DEBIAN/postinst"
chmod 755 "$DEB/DEBIAN/postinst"
