...
This chart deploys the hyper2kvm operator on a Kubernetes cluster using Helm. The operator provides:
...
|-----------|-------------|---------|
| `operator.image.repository` | Operator image repository | `ghcr.io/ssahani/hyper2kvm` |
...
kubectl port-forward -n hyper2kvm-system svc/hyper2kvm-operator-webhook 8080:8080
curl http://localhost:8080/metrics
```
...
hyper2kvm_operator_queue_depth{namespace="default"}
...
# View webhook logs
kubectl logs -n hyper2kvm-system -l app.kubernetes.io/component=webhook

# Verify webhook configurations
kubectl get validatingwebhookconfiguration hyper2kvm-operator-validating -o yaml
kubectl get mutatingwebhookconfiguration hyper2kvm-operator-mutating -o yaml
...
kubectl logs -n hyper2kvm-system -l app.kubernetes.io/component=operator | grep reconcile
...
- **Main Repository**: https://github.com/ssahani/hyper2kvm
